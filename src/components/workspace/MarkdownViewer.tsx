...
export function MarkdownViewer({ content, fileName, filePath, fileSize, fileModified, error, isStarred, onStar }: MarkdownViewerProps) {
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      {/* File header */}
      <div className="flex items-center gap-3 px-6 py-4 border-b border-shell-800 bg-shell-900/50 shrink-0">
        {/* Star button */}
        {filePath && onStar && (
          <button
        ...
        <div className="max-w-[1200px] mx-auto min-h-full">
          <div className="bg-shell-900 border border-shell-800 rounded-lg overflow-hidden p-4 min-h-full">
            ...
            <pre className="font-mono text-sm overflow-auto">
              <code 
                dangerouslySetInnerHTML={{ 
                  __html: Prism.highlight(content, Prism.languages[language] || Prism.languages.plain, language) 
                }} 
              />
            </pre>
          )}
